...
const createProject = async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Generate the image using image model
...
const createVideo = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!operation.response.generatedVideos)
      throw new Error(operation.response.raiMediaFilteredResponse[0]);
...
const getAllPublishedProjects = async (req: Request, res: Response) => {
  try {
...
const deletedProject = async (req: Request, res: Response) => {
  try {
    const { userId } = req.auth();
    if (!userId) return res.status(401).json({ message: "Unauthorized" });
...
export { createProject, createVideo, getAllPublishedProjects, deletedProject };
